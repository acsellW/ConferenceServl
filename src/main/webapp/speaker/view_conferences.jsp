    ...
    <div>
        <nav class="navbar navbar-expand bg-light">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath}/speaker/cabinet"><fmt:message
                            key="cabinet"/></a>
                </li>
            </ul>
        </nav>
...
<div class="container">
    ...
    <div>
        ...
        <c:if test="${currentPage != 1}">
            <td><a href="/speaker/view_conferences?page=${currentPage - 1}"><fmt:message key="previous"/></a></td>
        </c:if>
        <c:forEach begin="1" end="${noOfPages}" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <td><a href="/speaker/view_conferences?page=${i}">${i}</a></td>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:if test="${currentPage lt noOfPages}">
            <td><a href="/speaker/view_conferences?page=${currentPage + 1}"><fmt:message key="next"/></a></td>
        </c:if>
