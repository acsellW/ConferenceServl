    ...
    <div>
        <nav class="navbar navbar-expand bg-light">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath}/user/cabinet.jsp"><fmt:message
                            key="cabinet"/></a>
                </li>
            </ul>
        </nav>
...
<div class="container">
    ...
    <form class="form-inline my-2 my-lg-0" action="${pageContext.request.contextPath}/user/find" method="POST">
        <input class="form-control mr-sm-2" type="search" name="text" placeholder="<fmt:message key="title.search"/>"
               aria-label="Search">
...
<div class="container">
    ...
    <div>
        <table class="table table-striped">
            ...
            <tr>
                <th>
                    <a href="/user/view_conferences?page=${currentPage}&sort=title&sortDir=${sortDir eq 'asc' ? 'desc' : 'asc'}"><fmt:message
                            key="title"/></a></th>
                <th>
                    <a href="/user/view_conferences?page=${currentPage}&sort=date&sortDir=${sortDir eq 'asc' ? 'desc' : 'asc'}"><fmt:message
                            key="conferences.date"/></a></th>
            </tr>
            </thead>
            <tbody>
        ...
        <c:if test="${currentPage != 1}">
            <td><a href="/user/view_conferences?page=${currentPage - 1}&sort=${sort}&sortDir=${sortDir}"><fmt:message
                    key="previous"/></a></td>
        </c:if>
